...
bot = Bot(token='6738694633:AAEdaAeQ950YOBmTlJgJTraelVlHAPFOi6g', parse_mode=types.ParseMode.HTML)
        ...
        if len(ocr_result) > 0:
            try:
                ...
                await bot.send_message(uid,
                                       'Ошибка при распознавании! Попробуйте другое изображение или нажмите "В меню":',
...
async def handle_translate_text(message: types.Message, state):
    ...
    try:
        ...
        try:
            ...
            print(traceback.format_exc())
            await bot.send_message(uid,
                                   'Не удалось перевести текст! Попробуйте отправить другое предложение или нажмите "В меню":',
                                   reply_markup=to_menu_keyboard())
    except Exception:
        print(traceback.format_exc())
        await bot.send_message(uid, 'Что-то пошло не так...', reply_markup=initial_keyboard())
